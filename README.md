...
**OpenAI** rubric scoring → daily owner summary), and presents everything
...
| `packages/providers` | Provider interfaces; `mock/` demo implementations and `live/` clients (RingCentral, AgencyZoom, Deepgram, OpenAI) selected by `DATA_MODE` |
...
| `score_call` | on demand | Transcript → OpenAI rubric score (fixture score in demo), persisted to `call_scores` |
...
`RC_*`, `AZ_*`, `DEEPGRAM_*`, `OPENAI_*` groups. `PORT` is injected by
